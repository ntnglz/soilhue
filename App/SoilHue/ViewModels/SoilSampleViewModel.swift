...
enum SampleError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .noSampleSelected:
            return NSLocalizedString("sample.error.no.sample.selected", comment: "No sample selected error")
        case .analysisError:
            return NSLocalizedString("sample.error.analysis", comment: "Sample analysis error")
        case .saveError:
            return NSLocalizedString("sample.error.save", comment: "Sample save error")
        case .loadError:
            return NSLocalizedString("sample.error.load", comment: "Sample load error")
        }
    }
}
...
class SoilSampleViewModel: ObservableObject {
    ...
    func addSample(image: UIImage, location: CLLocation?) {
        ...
        print("DEBUG: SoilSampleViewModel - Imagen: \(image.size)")
        if let location = location {
            print("DEBUG: SoilSampleViewModel - Con ubicación: \(location.coordinate)")
        } else {
            print("DEBUG: SoilSampleViewModel - Sin ubicación")
        }
        
        let newSample = SoilSample(
            image: image,
            location: location
        )
        
        print("DEBUG: SoilSampleViewModel - Muestra creada con ID: \(newSample.id)")
        currentSample = newSample
        print("DEBUG: SoilSampleViewModel - currentSample configurado: \(String(describing: currentSample))")
    }
