...
struct ContentView: View {
    ...
    var mainContent: some View {
        ...
        .sheet(isPresented: $showImagePicker) {
            ImagePicker(image: $selectedImage, onSelectionComplete: { image in
                if let image = image {
                    // Configurar la muestra actual cuando se selecciona una imagen desde la galería
                    viewModel.addSample(image: image, location: nil)
                }
            })
        }
                ...
                capturedLocation: Binding(
                    ...
                    set: { location in
                        if let image = selectedImage {
                            // Configurar la muestra actual cuando se captura una imagen con la cámara
                            viewModel.addSample(image: image, location: location)
                        }
                    }
                ),
    ...
    var body: some View {
        ...
        .onChange(of: selectedImage) { _, image in
            if let image = image, viewModel.currentSample == nil {
                // Configurar la muestra actual cuando se selecciona una imagen
                viewModel.addSample(image: image, location: nil)
            }
        }
    }
        ...
        Task { @MainActor in
            if let data = try? await item.loadTransferable(type: Data.self),
               let image = UIImage(data: data) {
                selectedImage = image
                // Configurar la muestra actual cuando se selecciona una imagen desde el PhotosPicker
